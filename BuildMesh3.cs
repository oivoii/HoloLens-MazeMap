...
public class BuildMesh3 : MonoBehaviour {
    ...
    //hardcoded List used for testing, remove once the program gets coordinates as an input
    ...
    //List<List<List<double>>> inputVector = new List<List<List<double>>> { new List<List<double>> { new List<double> { 10.406320, 63.415900, 0 }, new List<double> { 10.406420, 63.415900, 0 }, new List<double> { 10.406420, 63.416000, 0 }, new List<double> { 10.406370, 63.416050, 0 }, new List<double> { 10.406320, 63.416000, 0 }, new List<double> { 10.406320, 63.415900, 0 } } };
    ...
    void Start () {
        ...
        //finding index of main room (most vertices)
        int roomIndex = 0;
        for (int i = 0; i < inputV.Count; i++)
        {
            if (inputV[roomIndex].Count < inputV[i].Count)
            {
                roomIndex = i;
            }
        }
        ...
        double degToRad = (Constants.Pi) / 180; // Used to convert from degrees to radians
        ...
        for (int i = 0; i < vectorLength; i++)
        {
            ...
            double sinPhi = Trig.Sin(phi);
            double sinTheta = Trig.Sin(theta);
            double cosPhi = Trig.Cos(phi);
            double cosTheta = Trig.Cos(theta);

            //double tempo = 1 - (c * (sinPhi * sinPhi));
            double tempo = 1 - (c * (sinPhi * sinPhi));

            double N = (rA / Sqrt(tempo));
            ...
            vertices[i][0] = (float)(1000 * (x - refX));
            vertices[i][1] = (float)(1000 * (y - refY));
            vertices[i][2] = (float)(1000 * (z - refZ));



            vertices[i + vectorLength][0] = vertices[i][0]; // Making vertices for the roof
            vertices[i + vectorLength][1] = vertices[i][1];
            vertices[i + vectorLength][2] = (float)(1000 * (z - refZ2));
        ...
        //double degToRad = Constants.Pi / 180;
        ...
        //    double longd_1 = inputVector[roomIndex][i][0];
        //    double lat_1 = inputVector[roomIndex][i][1];
        //    double longd_2 = inputVector[roomIndex][i + 1][0];
        //    double lat_2 = inputVector[roomIndex][i + 1][1];
            
        //    double l1 = longd_1 * degToRad;
        //    double mu1 = lat_1 * degToRad;
        //    double l2 = longd_2 * degToRad;
        //    double mu2 = lat_2 * degToRad;

            

        //    double dl = l2 - l1;

        //    double tmp1 = Trig.Sin(mu1) * Trig.Sin(mu2);
        //    double tmp2 = Trig.Cos(mu1) * Trig.Cos(mu2) * Trig.Cos(dl);
        //    double tmp3;
        //    double tmp4;
        //    double tmp5;

        //    double distance = Trig.Acos((tmp1) + (tmp2)) * R;

        //    //double distance = Trig.Acos((Trig.Sin(mu1) * Trig.Sin(mu2)) + (Trig.Cos(mu1) * Trig.Cos(mu2) * Trig.Cos(dl))) * R;

        //    double y = Trig.Sin(dl) * Trig.Cos(mu2);
        //    double x = (Trig.Cos(mu1) * Trig.Sin(mu2)) - (Trig.Sin(mu1) * Trig.Cos(mu2) * Trig.Cos(dl));

        //    double bearing;

        //    if (x > 0)
        //    {
        //        bearing = Trig.Atan(y / x);
        //    }
        //    else if (x < 0 & y >= 0)
        //    {
        //        bearing = Trig.Atan(y / x) + Constants.Pi;
        //    }
        //    else if (x < 0 & y < 0)
        //    {
        //        bearing = Trig.Atan(y / x) - Constants.Pi;
        ...
        //    vertices[i + 1][0] = (float)((distance * Trig.Cos(bearing)) + vertices[i][0]); 
        //    vertices[i + 1][2] = (float)((distance * Trig.Sin(bearing)) + vertices[i][2]);
        //    vertices[i + 1][1] = 0;
        ...
        for (int i = 0; i < vectorLength; i++)
        {
            vertices[i + vectorLength] = vertices[i];
        ...
        {
            if (maxValX < vertices[i].x) { maxValX = vertices[i].x; }
            else if (minValX > vertices[i].x) { minValX = vertices[i].x; }
            if (maxValY < vertices[i].y) { maxValY = vertices[i].y; }
            else if (minValY > vertices[i].y) { minValY = vertices[i].y; }
            if (maxValZ < vertices[i].z) { maxValZ = vertices[i].z; }
            else if (minValZ > vertices[i].z) { minValZ = vertices[i].z; }
        }
        ...
        for (int i = 0; i < vertices.Length; i++)
        {
            ...
            vertices[i].y = (int)(1000000 * (vertices[i].y - centerY));
        ...
        {
            tmp = indicesFloor[i + 1];
            indicesFloor[i + 1] = indicesFloor[i + 2];
            indicesFloor[i + 2] = tmp;
        }
        ...
        for (int i = 0; i < vertices.Length; i++)
        {
            vertices[i][0] = (((float)(vertices[i][0]) / 1000000) * scaleFactor);
            vertices[i][1] = (((float)(vertices[i][1]) / 1000000) * scaleFactor);
            vertices[i][2] = (((float)(vertices[i][2]) / 1000000) * scaleFactor);
        }
